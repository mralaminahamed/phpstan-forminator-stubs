...
# Generate Forminator stubs for all available versions
...
process_version() {
    ...
    if git rev-parse --quiet --verify "refs/tags/v${version}" >/dev/null; then
        log "Version ${version} already processed."
        return 0
    fi
    ...
    git tag "v${version}"
    log "Version ${version} processed successfully"
